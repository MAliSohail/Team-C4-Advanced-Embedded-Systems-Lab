...
def run_inspection(record: dict[str, Any]) -> None:
    ...
    try:
        ...
        for view, angle in enumerate(config.ANGLES, start=1):
            ...
            save_and_publish(record, completed_views=view)

        arduino.home()
        save_and_publish(
            record,
            state="WAITING_FOR_REVIEW",
            message="Select PASS or REJECT for every image",
            current_view=0,
            current_angle=0,
...
@app.post("/api/inspection/<inspection_id>/classify/<int:view>")
def classify(inspection_id: str, view: int):
    record = active_record(inspection_id)
    if record is None or record.get("state") not in {"WAITING_FOR_REVIEW", "REVIEW_IN_PROGRESS"}:
        return jsonify({"error": "inspection_not_ready"}), 409

    label = str((request.get_json(silent=True) or {}).get("label", "")).upper()
    ...
    view_data["classification"] = {
        "label": label,
        "source": "manual",
        "reviewed_at": now_iso(),
    }
    labels = [item["classification"] for item in record["views"]]
    reviewed = sum(item is not None for item in labels)

    if reviewed < 4:
        save_and_publish(
            record,
            state="REVIEW_IN_PROGRESS",
            message=f"{reviewed} of 4 images reviewed",
        )
    else:
        result = "REJECT" if any(item["label"] == "REJECT" for item in labels) else "PASS"
        save_and_publish(
            record,
            state="COMPLETE",
    ...
    return jsonify({"accepted": True, "state": record["state"], "final_result": record["final_result"]})
